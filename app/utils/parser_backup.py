...
class FileParser:
    ...
    def _process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if header_found:
            ...
            common_cols_indices = []
            for i, val in enumerate(row_days_raw):
            ...
            for day in unique_days:
                ...
                cols_to_select = common_cols_indices + current_day_indices
                chunk = data_body.iloc[:, cols_to_select].copy()

                # Derive new column names: common first, then attributes for current day
                new_col_names: List[str] = []
                for idx in common_cols_indices:
                    new_col_names.append(_name_for_common(idx))
                for idx in current_day_indices:
                    new_col_names.append(str(row_attrs.iloc[idx]).strip())

                chunk.columns = self._clean_columns(new_col_names)
                # Add Day column explicitly
                chunk['Day'] = day
                normalized_chunks.append(chunk)

            # Concatenate into a single normalized frame: Day | common_cols | attributes
            if normalized_chunks:
                ...
                # Move 'Day' first if present
                if 'Day' in df.columns:
                    df = df[['Day'] + [c for c in df.columns if c != 'Day']]
            else:
                # Fallback to slicing body if something went wrong
                df = df.iloc[header_idx + 2:]
        
        else:
            # --- Fallback: Standard Header ---
            # Assume first non-empty row is the header
            if not df.empty:
                df.columns = self._clean_columns(df.iloc[0])
                df = df.iloc[1:]
